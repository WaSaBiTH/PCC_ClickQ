  ...
  return (
    <nav className="bg-white border-b shadow-sm sticky top-0 z-40 mb-4 md:mb-8">
      <div className="max-w-6xl mx-auto px-2 sm:px-4 md:px-8 h-16 flex items-center justify-between gap-2 overflow-hidden">
        <Link href="/" className="font-bold text-base sm:text-lg md:text-xl hover:opacity-80 flex items-center gap-1 md:gap-2 truncate shrink-0">
        ...
        <div className="flex items-center gap-1 sm:gap-1 md:gap-2 overflow-x-auto custom-scrollbar flex-nowrap justify-start sm:justify-end pb-1 pt-1 -mb-1">
          <Link prefetch={false} href="/admin/dashboard" onClick={() => { if (activePage !== 'dashboard') setNavigatingAction('dashboard'); }}>
            <Button variant="ghost" size="icon" className={getButtonClass('dashboard')} title="Dashboard" disabled={navigatingAction === 'dashboard'}>
              {navigatingAction === 'dashboard' ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2 animate-spin" /> : <LayoutDashboard className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2" />}
          ...
          <Link prefetch={false} href="/admin/gallery" onClick={() => { if (activePage !== 'gallery') setNavigatingAction('gallery'); }}>
            <Button variant="ghost" size="icon" className={getButtonClass('gallery')} title="Manage Gallery" disabled={navigatingAction === 'gallery'}>
              {navigatingAction === 'gallery' ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2 animate-spin" /> : <ImageIcon className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2" />}
          ...
          <Link prefetch={false} href="/admin/team" onClick={() => { if (activePage !== 'team') setNavigatingAction('team'); }}>
            <Button variant="ghost" size="icon" className={getButtonClass('team')} title="Manage Team" disabled={navigatingAction === 'team'}>
              {navigatingAction === 'team' ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2 animate-spin" /> : <Users className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2" />}
          ...
          <Link prefetch={false} href="/admin/settings" onClick={() => { if (activePage !== 'settings') setNavigatingAction('settings'); }}>
            <Button variant="ghost" size="icon" className={getButtonClass('settings')} title="Settings" disabled={navigatingAction === 'settings'}>
              {navigatingAction === 'settings' ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2 animate-spin" /> : <Settings className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2" />}
          ...
          <Button variant="ghost" size="icon" className="lg:w-auto lg:px-4 text-red-600 hover:text-red-700 hover:bg-red-50" title="ออกจากระบบ" disabled={navigatingAction === 'logout'} onClick={async () => {
            ...
            {navigatingAction === 'logout' ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2 animate-spin" /> : <LogOut className="w-4 h-4 sm:w-5 sm:h-5 lg:mr-2" />}
