...
export function ClientLink({ children, className = "", loaderClassName = "w-4 h-4 mr-2 animate-spin", onClick, ...props }: ClientLinkProps) {
  ...
  return (
    <Link {...props} onClick={handleClick} className={`${className} ${isNavigating ? 'opacity-70 pointer-events-none' : ''}`}>
      {isNavigating && (
        <svg className={loaderClassName} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
